...
L1=9.5;
hc1=10;
Hvals=abs(((aml1+y(:,2))/2+tanh(y(:,1)/(L1*hc1)).*(aml1-y(:,2))/2)-0.6);
ii=find(Hvals==min(Hvals(:)));
y0=y(ii,1);
%Cp=(0.5*tanh(0.1*(y(:,1)-y0))+0.5).*((1-(0.2./a).^1000).*(1-y(:,2)./0.8));%(1-(a./(0.5*y(:,2)+0.5*0.8))).*(1-0.2./a);%.*((1-(y(:,2))./0.8));%.*exp(-0.0001.*(y(:,1)).^2);%*(0.5*tanh(y(:,1)./50)+0.5);%(1-.5*(y(:,2)+a)./0.8)%.*exp(-0.0001.*(y(:,1)).^2)%(1-a./y(:,2));%Ci.*max(0,(0.6-a)./0.6);%(0.5+0.5*tanh(0.6-a./0.6))%(1-a./(y(:,2)));
